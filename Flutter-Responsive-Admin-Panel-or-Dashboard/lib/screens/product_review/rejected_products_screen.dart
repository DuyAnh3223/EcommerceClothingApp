...
class RejectedProductsScreen extends StatefulWidget {
  final VoidCallback? onProductReviewed;

  const RejectedProductsScreen({Key? key, this.onProductReviewed}) : super(key: key);

  @override
  State<RejectedProductsScreen> createState() => _RejectedProductsScreenState();
}

class _RejectedProductsScreenState extends State<RejectedProductsScreen> {
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      final result = await PendingProductService.getProductsByStatus('rejected');
      if (result['success']) {
        setState(() {
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              if (product.reviewerName != null) Text('Người từ chối: ${product.reviewerName}'),
              if (product.reviewedAt != null) Text('Ngày từ chối: ${product.reviewedAt}'),
              if (product.reviewNotes != null && product.reviewNotes!.isNotEmpty)
                Text('Lý do từ chối: ${product.reviewNotes}'),
            ],
          ),
        ),
        actions: [
